...
- Log the failing response body in a more structured way for debugging.
...
P1: Preflight name-slug conflict before quiz start
- Check the normalized `nameSlug`, not just the raw display name.
- Surface a precise message before the first question is shown.
...
- Validate the normalized name against the effective uniqueness rule before the quiz starts.
...
- A fingerprint conflict is distinguishable from replay and from generic save failure.
...
1. Patch client messaging and eligibility gating
2. Patch normalized-name preflight validation
